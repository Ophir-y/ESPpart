...
bool ReadCard(byte u8_UID[8], kCard* pk_Card){
    ...
    if (pk_Card->e_CardType != CARD_Unknown){	// In our case we must get into this condition
...
bool IsDesfireTimeout(){
    ...
    if (nfc_PN532.GetLastPN532Error() == 0x01){ // Timeout
...
int TrigerRfid(){
	...
	if (!ReadCard(k_User.ID.u8, &k_Card)){
		if (IsDesfireTimeout());//Prints the error inside the function
	...
	if(k_Card.u8_UidLength == 4){
	...
	else if(k_Card.u8_UidLength == 7){		//Desfire
		...
		while(e_Status == ST_MoreFrames){
			...
			if(ptr - dataBuffer > FILE_SIZE_IN_BYTES){ // Sanity check to prevent segmentation fault
				SerialClass::Print("Error\r\n");
				return -1;		
			}
